// ICP: Algorithm with only auxiliary filer,
//		invariant-centroid-based pruning.
//-----------------------------------------//
#include	<stdio.h>
#include	<stdlib.h>
#include	<math.h>
...
int	calMeans_sparse_1st(int i, double all_distcalc)
{
	...
	int	th_id;//Thread ID
	int	ctrI;//Counter for invariant centroids
...
#pragma omp parallel num_threads(NumThreads) private(th_id,k,t,h,init_cid,final_cid,ass_sim,tid,v) reduction(+:num_mult,num_chobj,obj_funct,num_obsim)
{
	...
	for(j = 0; j < N; j++){// Outermost loop 
		...
		for(t = 0; t < vNT[j]; t++){
			tid = mTID[j][t]; 
			for(h = 0; h < vNM[tid]; ++h){
				k = mMID[tid][h]; mAppear[th_id][k] = 1;
...
#pragma omp parallel num_threads(NumThreads) private(j)
{
	...
	for(k = 0; k < K; k++){
		...
		for(tid = 0; tid < NumUTD; ++tid)
			if(vTcount[tid] == 1){vNM[tid]++; vTcount[tid] = 0;}
	}//for-K
...
#pragma omp parallel num_threads(NumThreads) private(tid,j,h,k,v,ctrI)
{
	#pragma omp for
	for(tid = 0; tid < NumUTD; tid++){
		ctrI = vNM[tid] -1;
		for(h = 0; h < vNM[tid]; ++h){
			...
			//if(h == ctrI && vNew[k] == 1){break;}
			//else if(h == ctrI && vNew[k] == 0){ctrI--; break;}
...
int	calMeans_sparse(int i, double all_distcalc)
{
	...
	double	v, w;
	double	obj_funct;//Objective function value
	int	id, tid;//Object_ID, Term_ID, Partial Term_ID
	int	init_cid, final_cid;
	long num_obsim;//Number of multiplications for sim. calc.
	unsigned long num_mult;//Number of multiplications for sim. calc.
	int	num_chobj;//Number of objects whose assignments change
	...
	start_time = omp_get_wtime();
	//-- Object assignment --//
#pragma omp parallel num_threads(NumThreads) private(th_id,j,k,t,h,v,init_cid,final_cid,ass_sim,tid) reduction(+:num_mult,num_chobj,obj_funct,num_obsim)
{
	th_id = omp_get_thread_num();
	#pragma omp for
	for(j = 0; j < N; j++){// outermost loop
		...
		if(vObjStatus[j] == 0){//Invariant oe near-invariant objects
			for(t = 0; t < vNT[j]; ++t){// middle loop
				tid = mTID[j][t];
...
#pragma omp parallel num_threads(NumThreads) private(j,h,id,tid,ass_sim)
{
		...
		for(j = 0; j < vClstSize[k]; ++j){
			...
			if(ass_sim >= vAssSim[id]) vObjStatus[id] = 0;
			//See several lines below for-k loop w.r.t vNew[k]
			vAssSim[id] = ass_sim;
		}
}//omp
...
void	memFree()
{
	...
	free(mMean); free(mMID); free(vNM); free(vNMvM);
...
int	main(int argc, char **argv)
{
	...
	strcpy(db_file,argv[5]);
	strcpy(assign_file,argv[6]);
	strcpy(info_file,argv[7]); 
